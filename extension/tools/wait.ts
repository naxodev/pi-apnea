...
import { asVerdict, isCompleteArtifact, readArtifact } from "../lib/frontmatter.ts";
import { abs, herdrEnabled, paneGet, findPaneByLabel, roleLabel, sleepMs } from "../lib/herdr-wait.ts";
import { err, ok } from "../lib/result.ts";
import { assertToolAllowed, stepAfterArtifact, type DispatchKind } from "../lib/state-machine.ts";
...
function inferKind(artifactRel: string): DispatchKind {
	if (artifactRel.endsWith("plan.md") && !artifactRel.includes("plan-review")) return "plan";
...
export function workflowWait(params: {
	...
	while (Date.now() < deadline) {
		...
		if (isCompleteArtifact(fm, { requireVerdict })) {
			...
			if (state.pending_role === "reviewer" && state.reviewer_tree_fingerprint != null) {
	...
	saveState(state, root);
	return err(`timeout after ${timeout}ms waiting for ${state.pending_artifact}`, {
		data: {
